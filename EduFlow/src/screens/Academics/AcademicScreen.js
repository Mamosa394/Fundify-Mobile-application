  ...
  // 3D Brain Model - Same pattern as BookModel in AssignmentScreen
  function BrainModel() {
    ...
    return (
      <>
        ...
        <primitive ref={groupRef} object={scene} scale={0.08} position={[-0.011, 0.001, -29]} />
      </>
    );
  }
  ...
  function PieChart({ data, size = 120 }) {
    ...
    return (
      <Svg width={size} height={size}>
        ...
        <SvgCircle cx={center} cy={center} r={radius * 0.55} fill="white" />
      </Svg>
    );
  }
  ...
  function LineChart({ data, width: chartWidth = SCREEN_WIDTH - 72, height = 100 }) {
    ...
    return (
      <Svg width={chartWidth} height={height}>
        <Line x1={padding} y1={padding + h} x2={padding + w} y2={padding + h} stroke={COLORS.shadow} strokeWidth={1} />
        <Line x1={padding} y1={padding} x2={padding} y2={padding + h} stroke={COLORS.shadow} strokeWidth={1} />
        {data.map((d, i) => {
          const x = padding + (i / Math.max(data.length - 1, 1)) * w;
          const y = padding + h - (d.value / maxVal) * h;
          return (
            <G key={i}>
              ...
              <SvgText x={x} y={padding + h + 14} fontSize={9} fill={COLORS.textMuted} textAnchor="middle">{d.label}</SvgText>
            </G>
          );
        })}
  ...
  const AcademicScreen = () => {
    ...
    const lineData = [
      { label: 'W1', value: gpa > 0 ? Math.max(0, gpa - 0.5) : 2.5 },
      { label: 'W2', value: gpa > 0 ? Math.max(0, gpa - 0.3) : 2.8 },
      { label: 'W3', value: gpa > 0 ? Math.max(0, gpa - 0.1) : 3.0 },
      { label: 'Now', value: gpa || 3.0 },
    ];
      ...
      return (
        <LinearGradient colors={[COLORS.bgStart, COLORS.bgMid, COLORS.bgEnd]} style={styles.container}>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptySubtitle}>Add your first module to unlock analytics</Text>
    ...
    return (
      <LinearGradient colors={[COLORS.bgStart, COLORS.bgMid, COLORS.bgEnd]} style={styles.container}>
        ...
        >
          {/* 3D Brain + GPA - Same pattern as AssignmentScreen */}
          <View style={styles.canvasContainer}>
            {isFocused && (
              <Canvas dpr={1} gl={{ antialias: true, alpha: true }} camera={{ position: [-100, 20, 0], fov: 50 }}>
                <Suspense fallback={null}>
                  <BrainModel />
                </Suspense>
              </Canvas>
            )}
            <View style={styles.gpaOverlay}>
          ...
          <View style={styles.statsRow}>
            {[
              ...
              { label: 'Completed', value: `${completionRate}%`, icon: CheckCircle2, color: COLORS.success },
          ...
          <View style={styles.card}>
            ...
            <View style={styles.progressLabels}>
              <Text style={styles.progressText}>{completedAssignments} of {totalAssignments} completed</Text>
  ...
  const styles = StyleSheet.create({
    ...
    // Canvas with overlay - Same pattern as AssignmentScreen
    canvasContainer: { height: 220, borderRadius: 20, overflow: 'hidden', marginBottom: 16, backgroundColor: '#1e293b', position: 'relative' },
    ...
    progressPercent: { fontSize: 13, fontFamily: 'JosefinSans-Bold', color: COLORS.text },
